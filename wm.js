...
function addLogin() {
  ...
  var win = confirm("Enter your new username & password", function () {
    setUserDetails(usernameBox.value, passwordBox.value);
  });
...
function setUserDetails(username, pass) {
  ...
  passHashTo(pass, function (h) {
    localStorage.passwordHash = h;
  });
}
...
function dragElement(elmnt) {
  ...
  $(elmnt.id + "header").addEventListener("dbclick", function () {
    repos(elmnt, 0, 30);
    resize(elmnt, "100%", "calc(100% - 30px)");
  });
  ...
  try {
    navigator.getBattery().then(function (battery) {
      function updateIcon() {
        ...
        battery_button.title =
          battery.level * 100 + "%" + (battery.charging ? " - charging" : "");
        if (!battery.charging) {
          if (battery.level == 1) {
            ...
            battery_button.classList.add("white-icon");
            battery_button.src = getIcon(
              "battery-level-" + Math.round(battery.level * 10) * 10
            );
    ...
    function time() {
      ...
      var hour = d.getHours();
      document.getElementsByClassName("activity-bar")[0].innerHTML =
        month +
        " " +
        date +
        "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
...
error = function (text) {
  makeAlert(text, "destructive");
};
console.error = error;

window.onerror = function (text, url, line, col, err) {
  error("line " + line + ", col " + col + ": " + text);
};

warn = function (text) {
  makeAlert(text, "warning");
};
